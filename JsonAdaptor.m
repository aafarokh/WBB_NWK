...
% modified on July 9th 2021 and can be found at:
...
for jk=1:sheetnum  
    disp(sheets{jk})
    colheaders=MTstruct.textdata.(sheets{jk})(1,:);
    directorypath=['C:\Users\amirali.farokhniaee\Desktop\Processed_Signals\',sheets{jk},'\'];
    if ~exist([directorypath,'JSONs'],'dir') %check if a folder exist to save JSON files
        mkdir([directorypath,'JSONs'])
    end
    new_filepath=[directorypath,'JSONs\'];
    ...
    for i=2:size(MTstruct.textdata.(sheets{jk}),1)-7 %-7 because there are footnote commetns on all sheets
        %  for i=10
        disp(MTstruct.textdata.(sheets{jk}){i,1})
        entire_row_num=MTstruct.data.(sheets{jk})(i-1,:);
        entire_row_text=MTstruct.textdata.(sheets{jk})(i,:);
